# AXO Audit Report: www.clocktowerassoc.com
...
| AXO Landing | 18.2 KB* | **59.5 KB** |
| Services | 2.2 KB | **22.1 KB** |

*The AXO page had `getStaticProps` data serialized in `__NEXT_DATA__`, inflating the response size despite the empty DOM.

### Score Impact

Without this fix, Content Survivability would have scored **0/100** — the textbook "SPA Blank Page" failure pattern described in Section 7.1 of the AXO Framework specification.

```
Pre-fix AXO  = (0 × 0.25) + (72 × 0.20) + (78 × 0.20) + (30 × 0.15) + (68 × 0.10) + (76 × 0.10)
...
| /axo | 2,631 | Yes (18,253 chars) | SSG (getStaticProps) |
| /axo/framework | 4,621 | Yes (33,987 chars) | SSG (getStaticProps) |
...
Clean sequential hierarchy on all pages except /axo. Representative examples:
...
| SL-3 | Two `<h1>` elements on /axo — embedded business case markdown introduces a second document heading | Medium |
...
| IMC-3 | 2 `onclick` attributes on /axo page | Low |
| IMC-4 | Dark mode toggle SVGs missing `aria-hidden="true"` (2 per page) | Low |
| IMC-5 | Decorative arrow SVGs in links missing `aria-hidden="true"` on /services and /axo | Low |
...
| /services | 0 | 0 | 2 | 0 |
| /axo | 0 | 2 | 6 | 2 |
| /axo/framework | 20 | 0 | 6 | 2 |
| /about | 0 | 0 | 2 | 0 |
| /contact | 0 | 0 | 2 | 0 |

### Table Accessibility (/axo/framework only)
...
| DE-4 | Tables on /axo/framework lack `<caption>` and `scope` attributes | Medium |
| DE-5 | `<dl>`/`<dt>`/`<dd>` used only on /axo (2 definition lists) — unused elsewhere for attribute data | Low |

### Score Justification

No structured data markup of any kind. An agent can read the text content but cannot programmatically extract Organization identity, service offerings, contact information, or page hierarchy as typed data. The content is human-readable but not machine-parseable beyond raw text extraction. Tables exist on the framework page but lack full metadata. The `<dl>` usage on /axo is a positive signal but isolated.
...
|---|---|---|
| /axo/framework | Yes (`<nav aria-label="Breadcrumb">`) | No |
| /axo/axiom-specification | Yes (`<nav aria-label="Breadcrumb">`) | No |
...
| NT-3 | Sitemap missing 7 live pages (/axo, /axo/framework, /axo/axiom-specification, /showcase/*) | High |
...
```
Clocktower and Associates Clocktower Home Services AXO & AXIOM Tools
...
| ARF-2 | Two `<h1>` elements on /axo create ambiguity in page topic identification | Medium |
...
```
Post-fix AXO = (92 × 0.25) + (72 × 0.20) + (78 × 0.20) + (30 × 0.15) + (68 × 0.10) + (76 × 0.10)
             = 71.9 → 72
...
| 4 | **Fix the /axo dual h1** — Downgrade embedded business case h1 to h2 or suppress heading level in the rendering wrapper | Structural Legibility, Agent Response Fitness | +3–5 points across SL/ARF; ~+1 composite |
...
*This report was generated using the AXO Framework v3.0 scoring methodology. The site audited is our own — because if you're going to sell agent-readiness audits, you should be able to pass one.*
